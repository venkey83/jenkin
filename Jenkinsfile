...
pipeline {
    ...
    parameters {
        ...
        string(name: 'region', defaultValue: 'eu-west-1', description: 'select region to deployment')
            ...
            steps {
                sh '''
                  terraform init \
                      ...
                      -force-copy \
                      -backend=true \
                      -backend-config "bucket=terraform-state-bucket-vk" \
                      -backend-config "key=terraform-${region}/${service}.tfstate" \
                      -backend-config "region=${region}" \
                      -backend-config "dynamodb_table=dev-terraform" \
                      -lock=true
                '''
                sh """#!/bin/bash
